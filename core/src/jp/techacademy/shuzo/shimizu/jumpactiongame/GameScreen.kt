package jp.techacademy.shuzo.shimizu.jumpactiongame
...
class GameScreen(private val mGame: JumpActionGame) : ScreenAdapter() {
    ...
    private var mEnemys: ArrayList<Enemy>//課題
    ...
    private fun createStage() {
        ...
        while (y < WORLD_HEIGHT - 5) {
            ...
            if (mRandom.nextFloat() > 0.5f) {
                val enemy = Enemy( enemyTexture, 0, 0, 328, 328)
                val x = 2*mRandom.nextFloat() * (WORLD_WIDTH - Step.STEP_WIDTH)
                 enemy.setPosition(x, step.y + Star.STAR_HEIGHT + mRandom.nextFloat() * 4)
                mEnemys.add(enemy)
    ...
    private fun checkCollision() {
        ...
        for (i in 0 until mEnemys.size) {
            ...
            if (stepA.mEnemy == Step.STEP_STATE_VANISH) {
                continue
            }

            if (mPlayer.y > stepA.y) {
                if (mPlayer.boundingRectangle.overlaps(stepA.boundingRectangle)) {
                    mPlayer.hitStep()
                    if (mRandom.nextFloat() > 0.5f) {
                        val music = Gdx.audio.newMusic(Gdx.files.internal("ankle-spoil1.mp3"))
                        music.play()

                        //val sound = Gdx.audio.newSound(Gdx.files.internal("ankle-spoil1.mp3"))
                        //sound.play(3.0f)
                        mGameState = GAME_STATE_GAMEOVER
                    }
                    break
